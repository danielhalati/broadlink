...
if __name__ == '__main__':
        ...
        for device in devices:
            if device.mac.hex() ==  daniBroadlinkMAC:
                setup('Fane', '12345678', 1)
                device.auth()
            if learn == True:
                recordRF(device, 'recordTest.txt')
            else:
                #for i in range (0,5):
                replay(device, 'recordTest.txt')